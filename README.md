...
Méthodologie
Feature engineering client : RFM enrichi (récence, fréquence, montant total/moyen, volatilité, fréquence hebdomadaire, répartition par type de transaction) — 12 variables par client.
Traitement des valeurs aberrantes : winsorisation 1%/99%, documentée, aucun client exclu.
Transformation & normalisation : log1p sur variables monétaires/comptage (distributions à queue lourde) + StandardScaler (Z-score).
ACP : exploration de la structure, visualisation 2D.
Clustering K-means : choix de K par coude + silhouette, contraint à une plage business exploitable (3 à 7 segments) pour éviter les solutions dégénérées (2 méga-groupes ou sur-fragmentation).
Classification Hiérarchique Ascendante (Ward) : validation croisée via indice de Rand ajusté.
Caractérisation & nommage : profils par segment, règles métier, personas.
Recommandations marketing : offre, canal, message clé et KPI par segment.
